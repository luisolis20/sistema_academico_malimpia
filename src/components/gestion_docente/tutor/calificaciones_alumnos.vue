    ...
    <div class="container-fluid py-4 bg-light min-vh-100">
        <div class="card border-0 shadow-sm rounded-4 mb-4 animate__animated animate__fadeIn">
            <div class="card-body p-0">
                <div class="row g-0">
                    <div class="col-md-4 bg-blue text-white p-4 d-flex flex-column justify-content-center rounded-start-4 position-relative overflow-hidden">
                        <div class="position-relative z-1">
                            <h3 class="fw-bold mb-1" style="font-family: 'Fraunces';">Panel del docente tutor</h3>
                            <h1 class="fw-extrabold text-gold mb-3" style="font-family: 'Fraunces';">Reporte de Calificaciones</h1>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
                        <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center border-bottom pb-3 gap-2">
                            <h5 class="fw-bold text-blue mb-0">
                                <i class="fas fa-users me-2 text-gold"></i>Calificaciones de mis Alumnos
                            </h5>
                            <div class="d-flex flex-wrap gap-2">
                                <button @click="generarCertificadosMasivosAprobacion" class="btn btn-gold text-blue btn-sm rounded-pill px-3 shadow-sm fw-bold">
                                    <i class="fas fa-file-pdf me-1"></i> Certificado Masivo de Aprobación
                                </button>

                                <div class="input-group input-group-sm w-auto">
                                    <span class="input-group-text bg-light border-0"><i class="fas fa-search text-muted"></i></span>
                                    <input type="text" v-model="searchQuery" class="form-control border-0 bg-light" placeholder="Buscar alumno...">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle custom-table text-center">
                                ...
                                <tbody>
                                    <tr v-for="alumno in alumnosFiltrados" :key="alumno.id_matricula" class="transition-all">
                                        <td class="ps-3 py-3 text-start">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div>
                                                    <div class="fw-bold text-blue fs-6">{{ alumno.estudiante.apellidos }} {{ alumno.estudiante.nombres }}</div>
                                                    <small class="text-gold fw-bold x-small">{{ alumno.estudiante.cedula }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="d-flex flex-column align-items-center justify-content-center">
                                                <span class="fw-bold mb-1 x-small" :class="getColorTextoAsistencia(alumno.porcentaje_asistencia)">
                                                    {{ alumno.porcentaje_asistencia }}%
                                                </span>
                                                <div class="progress shadow-sm" style="width: 60px; height: 6px; background-color: #e9ecef;">
                                        ...
                                        <td>
                                            <span class="badge px-3 py-1.5 rounded-pill fw-bold" 
                                                ...
                                                <i class="fas me-1" :class="alumno.estado === 'APROBADO' ? 'fa-check' : 'fa-times'"></i>
                                                {{ alumno.estado }}
                                            </span>
                                        </td>
                                        <td>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div v-for="(top, index) in rankingEstudiantes" :key="top.id_matricula" 
                            ...
                            <div class="position-absolute end-0 top-0 p-2 opacity-25">
                                <i class="fas fa-medal fa-3x" :class="index === 0 ? 'text-warning' : (index === 1 ? 'text-secondary' : 'text-danger')"></i>
                            </div>

                            <div class="d-flex align-items-center position-relative z-1">
                                <span class="fw-black fs-4 me-3 text-blue-soft" style="width: 20px;">#{{ index + 1 }}</span>
                                <img :src="getPhotoUrl(top.estudiante.foto)" class="rounded-circle border border-2 me-2" width="42" height="42" style="object-fit: cover;">
                                <div>
                                    ...
                                    <small class="badge bg-gold text-blue x-small fw-bold">Promedio: {{ top.nota_final.toFixed(2) }}</small>
                                </div>
                            </div>

                            <button @click="generarCertificadoExcelencia(top)" class="btn btn-outline-blue btn-xs rounded-pill px-2.5 shadow-sm fw-bold position-relative z-1">
        ...
        <div class="modal fade" id="modalNotas" static tabindex="-1" aria-labelledby="modalNotasLabel" aria-hidden="true">
                ...
                <div class="modal-content border-0 shadow rounded-4">
                    ...
                    <div class="modal-body p-4" v-if="selectedAlumno">
                        <div class="d-flex align-items-center mb-4 pb-3 border-bottom">
                            <img :src="getPhotoUrl(selectedAlumno.estudiante.foto)" class="rounded-circle border border-3 border-blue-soft me-3" width="60" height="60" style="object-fit: cover;">
                            <div>
                                <h5 class="fw-bold text-blue mb-0">{{ selectedAlumno.estudiante.apellidos }} {{ selectedAlumno.estudiante.nombres }}</h5>
                                <small class="text-muted fw-semibold">Cédula: {{ selectedAlumno.estudiante.cedula }}</small>
                            </div>
                        </div>
                            ...
                            <table class="table table-sm table-striped align-middle text-center custom-table-sm mb-0">
                                ...
                                <tbody>
                                    <tr v-for="nota in selectedAlumno.calificaciones" :key="nota.asignatura">
                                        <td class="text-start ps-2 fw-semibold text-blue-light py-2">{{ nota.asignatura }}</td>
                                        <td class="fw-bold">{{ nota.nota_final.toFixed(2) }}</td>
                                        <td>
                                            <span class="badge rounded-pill px-2.5 py-1 text-uppercase x-small font-weight-bold"
                        ...
                        <div class="mt-3 p-3 bg-light rounded-3 d-flex justify-content-between align-items-center border-start border-blue border-3">
                            <span class="fw-bold text-blue small">PROMEDIO GENERAL ACUMULADO:</span>
...
.text-blue-light { color: #2c3e8c !important; }
.bg-blue { background-color: #1D2A68 !important; }
.text-blue-soft { color: rgba(29, 42, 104, 0.1) !important; }
.border-blue-soft { border-color: rgba(29, 42, 104, 0.15) !important; }
...
tr.transition-all:hover { background-color: rgba(244, 179, 36, 0.04) !important; }

/* Botones */
.btn-blue { background-color: #1D2A68; color: white; border: 1px solid #1D2A68; }
.btn-blue:hover { background-color: #121b44; color: white; }
.btn-gold { background-color: #F4B324; color: #1D2A68; border: none; }
.btn-gold:hover { background-color: #e0a216; color: #1D2A68; }
.btn-outline-blue { color: #1D2A68; border-color: #1D2A68; background-color: transparent; }
...
.text-warning.text-darken { color: #d39e00 !important; } /* Amarillo más oscuro para legibilidad */
.deco-circle { position: absolute; width: 200px; height: 200px; background: rgba(244, 179, 36, 0.1); border-radius: 50%; top: -50px; left: -100px; z-index: 0; }
